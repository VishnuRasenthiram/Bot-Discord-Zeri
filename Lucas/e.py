...
def find_parallelepiped_in_chunk(points_chunk, points_set, pairs):
    for (x1, y1, z1) in points_chunk:
...
def find_parallelepiped(points):
    ...
    # Diviser les points en chunks pour le multithreading
    num_threads = 8
    chunk_size = len(points) // num_threads
    chunks = [points[i:i + chunk_size] for i in range(0, len(points), chunk_size)]
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(find_parallelepiped_in_chunk, chunk, points_set, pairs) for chunk in chunks]
...
if parallelepiped:
    ...
    ax.scatter(x_points, y_points, z_points, s=100, label='Points du parallélépipède', c='r')
