...
async def creerImageCG(cg, regionId, region):
    print("1 - Début de la fonction creerImageCG")

    size = 1920, 1080
    sizeChamp = 308, 400
    link = f'https://ddragon.leagueoflegends.com/cdn/{version["v"]}/data/fr_FR/champion.json'
    
    print("2 - Ouverture du lien champion.json")
    f = urllib.request.urlopen(link)
    myfile = f.read()
    data = json.loads(myfile)
    champ = data["data"]
    
    print("3 - Ouverture de l'image de fond")
    ...
    for i in range(len(cg["participants"])):
        print(f"4.{i} - Traitement du participant {i + 1}")
        ...
        print(f"5.{i} - Récupération du pseudo pour le participant {i + 1}")
        ...
        print(f"6.{i} - Recherche du champion pour le participant {i + 1}")
        for cle, valeur in champ.items():
            if int(valeur['key']) == int(cg["participants"][i]['championId']):
                champion = cle

        print(f"7.{i} - Recherche du classement pour le participant {i + 1}")
...
def getChampImage(puuid, Champ, pseudo, rank, div, lp, spell, region):
    ...
    print("13 - Image du champion prête")
    return finalImage.resize(sizeChamp)


def getRankIcon(puuid, rank, region):
    print("14 - Récupération de l'icône de rang")
