...
class ZeriMoney:
    ...
    async def update_daily(self):
        ...
        for user in userListe:
            if user[4] == 0:
                update_user_profile(
                    user[0], user[1], user[2], user[3], user[4], 0)

            else:
                update_user_profile(
                    user[0], user[1], user[2], user[3], 0, user[5])
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        if user[4] == 0:
            ...
            update_user_profile(user[0], newMoney,
                                user[2], user[3], 1, newDaily)
    ...
    async def verif_level_up(self, message):
        ...
        if user[3] >= xp_requis:
            ...
            update_user_profile(
                user[0], user[1]+newLevel*100, newLevel, newXP, user[4], user[5])
    ...
    async def pile_ou_face(self, interaction: discord.Interaction, mise: int, choix: str):
        ...
        update_user_profile(user[0], newMoney, user[2],
