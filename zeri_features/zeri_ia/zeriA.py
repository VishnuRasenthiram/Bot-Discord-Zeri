...
def generate_content(prompt: str, user) -> str:
    try:
        ...
        # Nettoyage de l'historique pour éviter le spam de "Zeri:"
        history = "\n".join(
            line for line in message_history if not line.strip().startswith("Zeri:")
        )

        contents = [
            "Tu es Zeri, le personnage de League of Legends. "
            "Réponds avec un ton amical et énergique, en phrases courtes et directes. "
            "Utilise des emojis avec modération. "
            "Ne commence jamais ta réponse par 'Zeri:', réponds directement, sans redire ton nom. "
            f"Voici ce que {user} t'a dit : {prompt} "
            f"Voici l'historique à ne jamais renvoyer : {history}"
        ]
        ...
        if retour.strip() not in message_history:
            message_history.append(retour.strip())
